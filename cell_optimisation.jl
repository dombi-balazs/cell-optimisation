...
function crossover(top_half_population, bottom_half_population)
    ...
    for i in 1:num_offspring
        parent1_idx = rand(1:size(top_genes, 1))
        parent2_idx = rand(1:size(top_genes, 1))
        parent1 = top_genes[parent1_idx, :]
        parent2 = top_genes[parent2_idx, :]
...
function cell_optimisation()
    ...
    println("Best fitness found: ", final_ranked_population[1, 6])
end
